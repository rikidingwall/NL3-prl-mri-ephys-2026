...
## DRL TRAINING
win_stay_model_determ_train <- glmmTMB(
  p_win_stay ~ genotype + (1 | mouse_id),
  data = model_df_determ_train,
  family = beta_family(link = "logit")
)

determ_train_model_summary <- tidy(win_stay_model_determ_train, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    Task_Stage = "DRL",
    Metric = "Win-Stay",
...
## STABLE DRL
win_stay_model_determ_stable <- glmmTMB(
  p_win_stay ~ genotype + (1 | mouse_id),
  data = model_df_determ_stable,
  family = beta_family(link = "logit")
)

determ_stable_model_summary <- tidy(win_stay_model_determ_stable, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    Task_Stage = "DRL OT",
    Metric = "Win-Stay",
    term = ifelse(term == "(Intercept)", "ReferenceWT", term)
...
## PRL TRAINING
win_stay_model_prob_train <- glmmTMB(
  p_win_stay ~ genotype + (1 | mouse_id),
  data = model_df_prob_train,
  family = beta_family(link = "logit")
)

prob_train_model_summary <- tidy(win_stay_model_prob_train, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    Task_Stage = "PRL",
    Metric = "Win-Stay",
    term = ifelse(term == "(Intercept)", "ReferenceWT", term)
...
## STABLE PRL
win_stay_model_prob_stable <- glmmTMB(
  p_win_stay ~ genotype + (1 | mouse_id),
  data = model_df_prob_stable,
  family = beta_family(link = "logit")
)

prob_stable_model_summary <- tidy(win_stay_model_prob_stable, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    Task_Stage = "PRL OT",
    Metric = "Win-Stay",
    term = ifelse(term == "(Intercept)", "ReferenceWT", term)
...
## PRL BASELINING
win_stay_model_prob_base <- glmmTMB(
  p_win_stay ~ genotype + (1 | mouse_id),
  data = model_df_prob_base,
  family = beta_family(link = "logit")
)

prob_base_model_summary <- tidy(win_stay_model_prob_base, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
## PRL PROBE 1
win_stay_model_prob_probe_1 <- glmmTMB(
  p_win_stay ~ genotype + (1 | mouse_id),
  data = model_df_prob_probe_1,
  family = beta_family(link = "logit")
)

prob_probe_1_model_summary <- tidy(win_stay_model_prob_probe_1, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
## PRL PROBE 2
win_stay_model_prob_probe_2 <- glmmTMB(
  p_win_stay ~ genotype + (1 | mouse_id),
  data = model_df_prob_probe_2,
  family = beta_family(link = "logit")
)

prob_probe_2_model_summary <- tidy(win_stay_model_prob_probe_2, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
## PRL PROBE 3
win_stay_model_prob_probe_3 <- glmmTMB(
  p_win_stay ~ genotype + (1 | mouse_id),
  data = model_df_prob_probe_3,
  family = beta_family(link = "logit")
)

prob_probe_3_model_summary <- tidy(win_stay_model_prob_probe_3, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
# Export to CSV
write.csv(glmm_summaries, "glmm_summaries_win_stay.csv", row.names = FALSE)
