...
## DRL TRAINING
alpha_model_determ_train <- glmmTMB(
  alpha ~ genotype + (1 | mouse_id),
...
determ_train_model_summary <- tidy(alpha_model_determ_train, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "DRL", Metric = "Alpha",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))

## STABLE DRL
alpha_model_determ_stable <- glmmTMB(
  alpha ~ genotype + (1 | mouse_id),
...
determ_stable_model_summary <- tidy(alpha_model_determ_stable, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "DRL OT", Metric = "Alpha",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))

## PRL TRAINING
alpha_model_prob_train <- glmmTMB(
  alpha ~ genotype + (1 | mouse_id),
...
prob_train_model_summary <- tidy(alpha_model_prob_train, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL", Metric = "Alpha",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))

## STABLE PRL
alpha_model_prob_stable <- glmmTMB(
  alpha ~ genotype + (1 | mouse_id),
...
prob_stable_model_summary <- tidy(alpha_model_prob_stable, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL OT", Metric = "Alpha",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))

## PRL BASELINING
alpha_model_prob_base <- glmmTMB(
  alpha ~ genotype + (1 | mouse_id),
...
prob_base_model_summary <- tidy(alpha_model_prob_base, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL Baselining", Metric = "Alpha",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))

## PRL PROBE 1
alpha_model_prob_probe_1 <- glmmTMB(
  alpha ~ genotype + (1 | mouse_id),
...
prob_probe_1_model_summary <- tidy(alpha_model_prob_probe_1, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL Probe 1", Metric = "Alpha",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))

## PRL PROBE 2
alpha_model_prob_probe_2 <- glmmTMB(
  alpha ~ genotype + (1 | mouse_id),
...
prob_probe_2_model_summary <- tidy(alpha_model_prob_probe_2, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL Probe 2", Metric = "Alpha",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))

## PRL PROBE 3
alpha_model_prob_probe_3 <- glmmTMB(
  alpha ~ genotype + (1 | mouse_id),
...
prob_probe_3_model_summary <- tidy(alpha_model_prob_probe_3, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL Probe 3", Metric = "Alpha",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))
...
# Export to CSV
write.csv(glmm_summaries, "glmm_summaries_alpha.csv", row.names = FALSE)
