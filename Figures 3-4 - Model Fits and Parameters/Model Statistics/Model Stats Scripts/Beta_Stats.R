...
# Create Gaussian check function

check_gaussian <- function(model, data, response) {
  res <- residuals(model, type = "pearson")
  
  sw <- try(shapiro.test(res), silent = TRUE)
  if (!inherits(sw, "try-error") && sw$p.value < 0.05) {
    return(FALSE)
  }
  return(TRUE)
}
...
## DRL TRAINING
determ_train_gaussian_model <- glmmTMB(
  beta ~ genotype + (1 | mouse_id),
  data = model_df_determ_train,
  family = gaussian(link = "log")
)

if (check_gaussian(determ_train_gaussian_model, model_df_determ_train, "beta")) {
  beta_model_determ_train <- determ_train_gaussian_model
  message("Gaussian model ok.")
} else {
  message("Gaussian not appropriate — used Gamma instead.")
  beta_model_determ_train <- glmmTMB(
    beta ~ genotype + (1 | mouse_id),
    data = model_df_determ_train,
...
determ_train_model_summary <- tidy(beta_model_determ_train, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "DRL", Metric = "Beta",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))

## STABLE DRL
determ_stable_gaussian_model <- glmmTMB(
  beta ~ genotype + (1 | mouse_id),
  data = model_df_determ_stable,
  family = gaussian(link = "log")
)

if (check_gaussian(determ_stable_gaussian_model, model_df_determ_stable, "beta")) {
  beta_model_determ_stable <- determ_stable_gaussian_model
  message("Gaussian model ok.")
} else {
  message("Gaussian not appropriate — used Gamma instead.")
  beta_model_determ_stable <- glmmTMB(
    beta ~ genotype + (1 | mouse_id),
    data = model_df_determ_stable,
    family = Gamma(link="inverse")
  )
...
determ_stable_model_summary <- tidy(beta_model_determ_stable, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "DRL OT", Metric = "Beta",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))


## PRL TRAINING
prob_train_gaussian_model <- glmmTMB(
  beta ~ genotype + (1 | mouse_id),
  data = model_df_prob_train,
  family = gaussian(link = "log")
)

if (check_gaussian(prob_train_gaussian_model, model_df_prob_train, "beta")) {
  beta_model_prob_train <- prob_train_gaussian_model
  message("Gaussian model ok.")
} else {
  message("Gaussian not appropriate — used Gamma instead.")
  beta_model_prob_train <- glmmTMB(
    beta ~ genotype + (1 | mouse_id),
    data = model_df_prob_train,
    family = Gamma(link="inverse")
  )
...
prob_train_model_summary <- tidy(beta_model_prob_train, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL", Metric = "Beta",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))


## STABLE PRL
prob_stable_gaussian_model <- glmmTMB(
  beta ~ genotype + (1 | mouse_id),
  data = model_df_prob_stable,
  family = gaussian(link = "log")
)

if (check_gaussian(prob_stable_gaussian_model, model_df_prob_stable, "beta")) {
  beta_model_prob_stable <- prob_stable_gaussian_model
  message("Gaussian model ok.")
} else {
  message("Gaussian not appropriate — used Gamma instead.")
  beta_model_prob_stable <- glmmTMB(
    beta ~ genotype + (1 | mouse_id),
    data = model_df_prob_stable,
    family = Gamma(link="inverse")
  )
...
prob_stable_model_summary <- tidy(beta_model_prob_stable, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL OT", Metric = "Beta",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))


## PRL BASELINING
prob_base_gaussian_model <- glmmTMB(
  beta ~ genotype + (1 | mouse_id),
  data = model_df_prob_base,
  family = gaussian(link = "log")
)

if (check_gaussian(prob_base_gaussian_model, model_df_prob_base, "beta")) {
  beta_model_prob_base <- prob_base_gaussian_model
  message("Gaussian model ok.")
} else {
  message("Gaussian not appropriate — used Gamma instead.")
  beta_model_prob_base <- glmmTMB(
    beta ~ genotype + (1 | mouse_id),
    data = model_df_prob_base,
    family = Gamma(link="inverse")
  )
...
prob_base_model_summary <- tidy(beta_model_prob_base, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL Baselining", Metric = "Beta",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))


## PRL PROBE 1
prob_probe_1_gaussian_model <- glmmTMB(
  beta ~ genotype + (1 | mouse_id),
  data = model_df_prob_probe_1,
  family = gaussian(link = "log")
)

if (check_gaussian(prob_probe_1_gaussian_model, model_df_prob_probe_1, "beta")) {
  beta_model_prob_probe_1 <- prob_probe_1_gaussian_model
  message("Gaussian model ok.")
} else {
  message("Gaussian not appropriate — used Gamma instead.")
  beta_model_prob_probe_1 <- glmmTMB(
    beta ~ genotype + (1 | mouse_id),
    data = model_df_prob_probe_1,
    family = Gamma(link="inverse")
  )
...
prob_probe_1_model_summary <- tidy(beta_model_prob_probe_1, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL Probe 1", Metric = "Beta",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))


## PRL PROBE 2
prob_probe_2_gaussian_model <- glmmTMB(
  beta ~ genotype + (1 | mouse_id),
  data = model_df_prob_probe_2,
  family = gaussian(link = "log")
)

if (check_gaussian(prob_probe_2_gaussian_model, model_df_prob_probe_2, "beta")) {
  beta_model_prob_probe_2 <- prob_probe_2_gaussian_model
  message("Gaussian model ok.")
} else {
  message("Gaussian not appropriate — used Gamma instead.")
  beta_model_prob_probe_2 <- glmmTMB(
    beta ~ genotype + (1 | mouse_id),
    data = model_df_prob_probe_2,
    family = Gamma(link="inverse")
  )
...
prob_probe_2_model_summary <- tidy(beta_model_prob_probe_2, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL Probe 2", Metric = "Beta",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))


## PRL PROBE 3
prob_probe_3_gaussian_model <- glmmTMB(
  beta ~ genotype + (1 | mouse_id),
  data = model_df_prob_probe_3,
  family = gaussian
)

if (check_gaussian(prob_probe_3_gaussian_model, model_df_prob_probe_3, "beta")) {
  beta_model_prob_probe_3 <- prob_probe_3_gaussian_model
  message("Gaussian model ok.")
} else {
  message("Gaussian not appropriate — used Gamma instead.")
  beta_model_prob_probe_3 <- glmmTMB(
    beta ~ genotype + (1 | mouse_id),
    data = model_df_prob_probe_3,
    family = Gamma(link="inverse")
  )
...
prob_probe_3_model_summary <- tidy(beta_model_prob_probe_3, effects = "fixed", conf.int = TRUE) %>%
  mutate(Task_Stage = "PRL Probe 3", Metric = "Beta",
         term = ifelse(term == "(Intercept)", "ReferenceWT", term))
...
# Export to CSV
write.csv(glmm_summaries, "glmm_summaries_beta.csv", row.names = FALSE)