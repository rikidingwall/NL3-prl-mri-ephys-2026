...
## DRL TRAINING
accuracy_model_determ_train <- glmmTMB(
  Accuracy ~ Genotype + (1 | Animal_ID),
  data = determ_df_train,
  family = beta_family(link = "logit")
)

determ_train_model_summary <- tidy(accuracy_model_determ_train, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    Task_Stage = "DRL",
    Metric = "Accuracy",
...
accuracy_model_determ_stable <- glmmTMB(
  Accuracy ~ Genotype + (1 | Animal_ID),
  data = determ_df_stable,
  family = beta_family(link = "logit")
)

print(accuracy_model_determ_stable)

determ_stable_model_summary <- tidy(accuracy_model_determ_stable, effects = "fixed", conf.int = TRUE)

determ_stable_model_summary <- determ_stable_model_summary %>%
  mutate(
    Task_Stage = "DRL OT",
    Metric = "Accuracy",
    term = ifelse(term == "(Intercept)", "ReferenceWT", term)
...
## PRL TRAINING
accuracy_model_prob_train <- glmmTMB(
  Accuracy ~ Genotype + (1 | Animal_ID),
  data = prob_df_train,
  family = beta_family(link = "logit")
)

prob_train_model_summary <- tidy(accuracy_model_prob_train, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    Task_Stage = "PRL",
    Metric = "Accuracy",
    term = ifelse(term == "(Intercept)", "ReferenceWT", term)
...
accuracy_model_prob_stable <- glmmTMB(
  Accuracy ~ Genotype + (1 | Animal_ID),
  data = prob_df_stable,
  family = beta_family(link = "logit")
)

print(accuracy_model_prob_stable)

prob_stable_model_summary <- tidy(accuracy_model_prob_stable, effects = "fixed", conf.int = TRUE)

prob_stable_model_summary <- prob_stable_model_summary %>%
  mutate(
    Task_Stage = "PRL OT",
    Metric = "Accuracy",
    term = ifelse(term == "(Intercept)", "ReferenceWT", term)
...
## PRL BASELINING
accuracy_model_prob_base <- glmmTMB(
  Accuracy ~ Genotype + (1 | Animal_ID),
  data = prob_df_base,
  family = beta_family(link = "logit")
)

prob_base_model_summary <- tidy(accuracy_model_prob_base, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
## PRL PROBE 1
accuracy_model_prob_probe_1 <- glmmTMB(
  Accuracy ~ Genotype + (1 | Animal_ID),
  data = prob_df_probe_1,
  family = beta_family(link = "logit")
)

prob_probe_1_model_summary <- tidy(accuracy_model_prob_probe_1, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
## PRL PROBE 2
accuracy_model_prob_probe_2 <- glmmTMB(
  Accuracy ~ Genotype + (1 | Animal_ID),
  data = prob_df_probe_2,
  family = beta_family(link = "logit")
)

prob_probe_2_model_summary <- tidy(accuracy_model_prob_probe_2, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
## PRL PROBE 3
accuracy_model_prob_probe_3 <- glmmTMB(
  Accuracy ~ Genotype + (1 | Animal_ID),
  data = prob_df_probe_3,
  family = beta_family(link = "logit")
)

prob_probe_3_model_summary <- tidy(accuracy_model_prob_probe_3, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
# Export to CSV
write.csv(glmm_summaries, "glmm_summaries_accuracy.csv", row.names = FALSE)
