...
determ_df_train <- determ_df_train %>%
  filter(!is.na(Response_Latency) & Response_Latency > 0)

response_model_determ_train <- glmmTMB(
  Response_Latency ~ Genotype + (1 | Animal_ID) + (1 | Session_Number),
  data = determ_df_train,
  family = Gamma(link = "log")
)

determ_train_model_summary <- tidy(response_model_determ_train, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    Task_Stage = "DRL",
...
response_model_determ_stable <- glmmTMB(
  Response_Latency ~ Genotype + (1 | Animal_ID) + (1 | Session_Number),
  data = determ_df_stable,
  family = Gamma(link = "log")
)

determ_stable_model_summary <- tidy(response_model_determ_stable, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    Task_Stage = "DRL OT",
    Metric = "Response Latency",
    term = ifelse(term == "(Intercept)", "ReferenceWT", term)
...
response_model_prob_train <- glmmTMB(
  Response_Latency ~ Genotype + (1 | Animal_ID) + (1 | Session_Number),
  data = prob_df_train,
  family = Gamma(link = "log")
)

prob_train_model_summary <- tidy(response_model_prob_train, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    Task_Stage = "PRL",
    Metric = "Response Latency",
    term = ifelse(term == "(Intercept)", "ReferenceWT", term)
...
response_model_prob_stable <- glmmTMB(
  Response_Latency ~ Genotype + (1 | Animal_ID) + (1 | Session_Number),
  data = prob_df_stable,
  family = Gamma(link = "log")
)

prob_stable_model_summary <- tidy(response_model_prob_stable, effects = "fixed", conf.int = TRUE) %>%
  mutate(
    Task_Stage = "PRL OT",
    Metric = "Response Latency",
    term = ifelse(term == "(Intercept)", "ReferenceWT", term)
...
response_model_prob_base <- glmmTMB(
  Response_Latency ~ Genotype + (1 | Animal_ID) + (1 | Session_Number),
  data = prob_df_base,
  family = Gamma(link = "log")
)

prob_base_model_summary <- tidy(response_model_prob_base, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
response_model_prob_probe_1 <- glmmTMB(
  Response_Latency ~ Genotype + (1 | Animal_ID) + (1 | Session_Number),
  data = prob_df_probe_1,
  family = Gamma(link = "log")
)

prob_probe_1_model_summary <- tidy(response_model_prob_probe_1, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
response_model_prob_probe_2 <- glmmTMB(
  Response_Latency ~ Genotype + (1 | Animal_ID) + (1 | Session_Number),
  data = prob_df_probe_2,
  family = Gamma(link = "log")
)

prob_probe_2_model_summary <- tidy(response_model_prob_probe_2, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
response_model_prob_probe_3 <- glmmTMB(
  Response_Latency ~ Genotype + (1 | Animal_ID) + (1 | Session_Number),
  data = prob_df_probe_3,
  family = Gamma(link = "log")
)

prob_probe_3_model_summary <- tidy(response_model_prob_probe_3, effects = "fixed", conf.int = TRUE) %>%
  mutate(
...
# Export to CSV
write.csv(glmm_summaries, "glmm_summaries_response_latency.csv", row.names = FALSE)
